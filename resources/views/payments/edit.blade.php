...
<form style="width:80%; margin:auto;" class="px-4 py-5 shadow-lg" action="{{route('payments.update', $payment)}}" method="POST">
  ...
  <div class="form-group my-3 mx-5">
    ...
    <input type="text" class="form-control" id="paid_date" name="paid_date" placeholder="Enter your paid date" value="{{$payment->paid_date}}">
  </div>
  <div class="form-group my-3 mx-5">
    <label for="amount" class="mb-2">Amount</label>
    <input type="text" class="form-control" id="amount" name="amount" placeholder="Enter your amount" value="{{$payment->amount}}">
  </div>
  <div class="mx-5 my-4">
